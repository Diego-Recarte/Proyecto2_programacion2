...
public final class InstaFeedUI extends JPanel {
    ...
    private void loadFeed() {
        ...
        try {
            ...
            feedPosts = manager != null ? manager.getFeedPosts(currentUser) : new ArrayList<>();
            ...
            JLabel error = new JLabel("No se pudo cargar el feed: " + ex.getMessage(), SwingConstants.CENTER);
    ...
    private JPanel createPostCard(String[] post, int index) {
        ...
        int imageHeight = 240;
        try {
            ImageIcon icon = fitFeedImage(imagePath, imageWidth, 450);
            image.setIcon(icon);
            image.setText("");
        ...
        if (!caption.isBlank()) {
            JLabel captionLabel = new JLabel("<html><div style='width:350px'><b style='color:#ff4500'>@"
                    + html(author) + "</b> <span style='color:white'>" + html(caption) + "</span></div></html>");
            ...
            int captionHeight = Math.max(30, captionLabel.getPreferredSize().height + 12);
    ...
    private JButton navButton(String symbol, String tooltip) {
        JButton button = iconButton(symbol, tooltip);
        button.setFont(new Font("Segoe UI Symbol", Font.PLAIN, 23));
        return button;
    }
